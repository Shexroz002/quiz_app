...
async def student_quiz_session_websocket(
    ...
    async with AsyncSessionLocal() as db:
        quiz_session = QuizSessionRepository(db)

        is_host = await quiz_session.get_for_host(session_id=session_id, host_id=user.id)

        if not is_host:
            await websocket.close(code=1008, reason="you are not host of this session")
            return
...
@quiz_sessions.websocket("/ws/quiz-sessions/{session_id}/participant")
async def student_quiz_session_websocket(
        websocket: WebSocket,
        session_id: int,
