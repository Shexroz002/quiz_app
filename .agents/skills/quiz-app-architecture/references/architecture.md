...
- `app/bot/models.py`: `TelegramQuizRoom` durably maps a managed `QuizSession` to one Telegram chat message and leaderboard delivery state. `TelegramSinglePlayerResultDelivery` is the private-chat outbox keyed uniquely by a finished `QuizAttempt`. `TelegramRoomAnalysisDelivery` is the same shape for room participants: one row per finished attempt, carrying each member's personal analysis to their own chat.
...
- `app/services/pdf/pdf_job_service.py:PDFJobService`: validates/stores uploads, creates `PDFJob`, and queues PDF or description tasks. Both paths commit the row before dispatching Celery so the worker never races the transaction.
...
- `app/bot/tasks.py`: defines Celery `telegram.recover_rooms`, retrying `telegram.maintain_room`, `telegram.deliver_single_player_result`, and `telegram.deliver_room_analysis`; recovers all three durable Telegram outboxes, invokes managed multiplayer finalization, and publishes Telegram messages.
...
- `app/bot/handlers/quiz.py`: both quiz-generation sources plus the generated-quiz open/review/start callbacks. The PDF branch downloads and adapts the document to `UploadFile`; the AI branch walks subject -> topic description -> question count through `QuizGenerationState` and calls `PDFJobService.create_job_by_description`. `_pdf_job_service` and `_watch_job` are shared by both branches, so every job ends on the same Redis progress watcher and the same completed-quiz keyboard. The open and review callbacks are owner-checked fallbacks used only when no HTTPS Web App URL exists.
...
- `app/bot/services/analysis_message.py`: the one Telegram analysis body, shared by both delivery paths. Without a leaderboard it renders the single-player message; with one it adds the room rank, the top placings and the room average. It also owns topic highlight selection (single-question topics are excluded as noise) and the `📈 Batafsil tahlil` Mini App button, which is omitted rather than fatal when no HTTPS Web App URL exists.
- `app/bot/services/analysis_view.py`: view model behind the Mini App analysis page. It composes `get_finished_attempt_result`, `single_player_error_analysis` and the room leaderboard into totals, per-topic rows, a difficulty breakdown computed from the question rows, per-question answers, and an optional room standing.
- `app/bot/services/room_analysis.py`: durable per-participant analysis for finished rooms. It owns outbox creation, delivery and the undeliverable-chat case: a blocked or never-opened chat closes the row instead of leaving it for the recovery scan to retry forever. `publish_room` creates the outbox rows in the same transaction that stamps `leaderboard_delivered_at`, and queues the Celery tasks only after that commit.
...
- `app/bot/utils/progress.py`: Redis job-to-message mapping, job snapshots, progress text/message edits, and async watcher lifecycle. A job with no `file_name` but a `description` renders the AI wording (topic and question count) instead of the PDF wording; both share the completed and failed messages. The completed message carries `generated_quiz_keyboard`, whose first button opens the Mini App review flow.
- `app/bot/utils/profile_photo.py` and `upload.py`: `StorageService`-compatible Telegram file adapters.
- `app/bot/keyboards/reply.py`: phone contact and persistent main-menu keyboards. `inline.py`: registration, catalog, duration, and WebApp keyboards and URL. `webapp_url(**params)` is the single HTTPS Mini App URL builder; `quiz_webapp_url` and `analysis_webapp_url` are its two callers, and the `mode` parameter (`WEBAPP_REVIEW_MODE`, `WEBAPP_ANALYSIS_MODE`) selects the Mini App flow, and `generated_quiz_keyboard` offers review before the single-player and friends flows. `quiz_room.py`: group waiting/start deep links plus private Mini App buttons for running rooms.
- `app/bot/states/__init__.py`: registration, quiz generation (PDF plus the three AI steps), and custom-duration FSM states.
- `app/bot/webapp`: Vite/React Telegram Mini App. `src/App.jsx` dispatches on the `mode` query parameter read by `src/utils/telegram.js:getMode` (`review`, `analysis`, else the quiz); `src/pages/AnalysisPage.jsx` renders the post-attempt Natija/Mavzular/Savollar tabs from `GET /bot/sessions/{id}/analysis/`; `src/pages/QuizPage.jsx` owns quiz UI/state and switches between existing single-player and managed-room flows, while `src/pages/ReviewPage.jsx` owns the post-generation question review that reuses the same question components and changes only the correct option. `src/api/quiz.js` authenticates and calls the corresponding APIs; `src/utils/telegram.js` reads and validates Telegram init/query data and seeds the initial theme from `Telegram.WebApp.colorScheme`. `src/components/RichText.jsx`, `Question.jsx`, `QuestionMedia.jsx`, `AnswerOptions.jsx`, `QuizProgress.jsx`, and `QuizTimer.jsx` own Markdown/KaTeX rendering, media, choices, progress, and deadline UI. `RichText.jsx` is the single owner of generated-content rendering: `remark-gfm` + `remark-breaks` + `remark-math` + `rehype-katex` render tables, per-line breaks and LaTeX, it wraps every Markdown table in a `.table-scroll` container and routes Markdown images through `mediaUrl`, and KaTeX runs with `throwOnError: false` so an unparseable formula degrades to its own source text. `src/utils/markdown.js` owns the pre-render contract the AI prompt implies and is unit tested in `markdown.test.js` (`npm test`): it normalizes `\(...\)`/`\[...\]` into `$`/`$$` math and escapes `*`/`~` outside math, code and fences, because `app/services/ai/promt.py` asks for LaTeX formulas and no other Markdown, so a bare `*` is multiplication that emphasis would otherwise swallow. `src/styles.css` carries the whole palette as tokens redefined under `:root[data-theme="light"]`; extracted diagrams are transparent line art, so `.media-image` always paints the `--media-plate` background.
...
- `migration/versions/20260913_0005_telegram_room_analysis.py`: creates the attempt-unique durable outbox for per-participant Telegram room analysis delivery.
...
- New generated quiz source: feed the existing `save_quiz_from_json` path and `PDFJob` progress lifecycle. A Telegram entry point for one should reuse `app/bot/handlers/quiz.py:_pdf_job_service` and `_watch_job` rather than re-wiring the status message.
...
- New Mini App question editing: extend `QuestionService`/`QuestionRepository` and call the existing `/api/v1/question` endpoints from the Mini App with the `/bot/auth/` token; do not add a parallel editing API under `/api/v1/bot`.
...
- `QuizSessionRepository.get_session_questions_with_answers` accepted a participant argument it never applied and always returned the session's first participant; it is now scoped to the requesting user. Check similar owner-scoped queries before reusing them for multi-participant sessions.
