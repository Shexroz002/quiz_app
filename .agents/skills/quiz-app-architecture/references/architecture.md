...
- `app/bot/handlers/quiz.py`: PDF download/adaptation to `UploadFile`, `PDFJobService` creation, Redis progress watcher, and generated-quiz open/review/start callbacks. The open and review callbacks are owner-checked fallbacks used only when no HTTPS Web App URL exists.
...
- `app/bot/utils/progress.py`: Redis job-to-message mapping, PDF job snapshots, progress text/message edits, and async watcher lifecycle. The completed message carries `generated_quiz_keyboard`, whose first button opens the Mini App review flow.
